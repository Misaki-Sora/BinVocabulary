...
public:
  ...
  virtual void toStream(std::ostream &out_str) const;
  /**
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::toStream(std::ostream &out_str) const
{          
    ...
    uint32_t nnodes = m_nodes.size() - 1;
    printf("nodes %d \n",nnodes);
    ...
    while(!parents.empty())
    {
        ...
        for(auto pit :parent.children)
        {
            ...
            F::toStream(child.descriptor,aux_stream);
            // add to parent list
